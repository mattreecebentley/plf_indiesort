...
// Compiler-specific defines used by colony:
	...
	#define PLF_INDSORT_FORCE_INLINE __forceinline
		...
		#define PLF_INDSORT_NOEXCEPT_MOVE_ASSIGNMENT(the_allocator) throw()
	#elif _MSC_VER == 1600
	...
	#define PLF_INDSORT_FORCE_INLINE // note: GCC creates faster code without forcing inline
	...
	#define PLF_INDSORT_DESTROY(the_allocator, allocator_instance, location) 			std::allocator_traits<the_allocator>::destroy(allocator_instance, location)
	#define PLF_INDSORT_ALLOCATE(the_allocator, allocator_instance, size, hint) 		std::allocator_traits<the_allocator>::allocate(allocator_instance, size, hint)
 	#define PLF_INDSORT_ALLOCATE_INITIALIZATION(the_allocator, size, hint) 				std::allocator_traits<the_allocator>::allocate(*this, size, hint)
	...
	#define PLF_INDSORT_DESTROY(the_allocator, allocator_instance, location) 			allocator_instance.destroy(location)
	#define PLF_INDSORT_ALLOCATE(the_allocator, allocator_instance, size, hint)	 		allocator_instance.allocate(size, hint)
	#define PLF_INDSORT_ALLOCATE_INITIALIZATION(the_allocator, size, hint) 				the_allocator::allocate(size, hint)
...
#include <limits>  // std::numeric_limits
	...
	#define PLF_INDSORT_SORT_FUNCTION std::sort
#endif
...
namespace plf
{
	...
	template <typename T> 
	struct plf_is_pointer 
	{
		static const bool value = false;
	};

	template <typename T> 
	struct plf_is_pointer<T*> 
	{
	...
	void indiesort(const iterator_type first, const iterator_type last, comparison_function compare, const std::size_t size)
	{
		...
		typedef typename plf::derive_type<plf_is_pointer<iterator_type>::value, iterator_type>::type		element_type;
		...
		// Now, sort the pointers by the values they point to (std::sort is default sort function):
		PLF_INDSORT_SORT_FUNCTION(sort_array, sort_array + size, sort_dereferencer<comparison_function, item_index_tuple>(compare));
	...
	template <class iterator_type, class comparison_function>
	inline void indiesort(const typename plf::enable_if_c<!plf_is_pointer<iterator_type>::value, iterator_type>::type first, const iterator_type last, comparison_function compare)
	{
	...
	{
		indiesort(first, last, less<typename plf::derive_type<plf_is_pointer<iterator_type>::value, iterator_type>::type>());
	}
	...
	{
		#ifdef PLF_INDSORT_DECLTYPE_SUPPORT
			if PLF_INDSORT_CONSTEXPR (!has_size_function<container_type>::value)
			{
	...
	{
		indiesort(container, less<typename container_type::value_type>());
	}
}

#undef PLF_INDSORT_SORT_FUNCTION
...
#undef PLF_INDSORT_DESTROY
#undef PLF_INDSORT_ALLOCATE
#undef PLF_INDSORT_ALLOCATE_INITIALIZATION
